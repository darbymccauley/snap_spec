...
class Discover_Spectrometer:
    ...
    def __init__(self, host=HOST, fpgfile=FPGFILE, transport=TRANSPORT, stream_1=STREAM_1, stream_2=STREAM_2, logger=None, acc_len=ACC_LEN, spec_per_acc=SPEC_PER_ACC, chip_sel=CHIP_SEL_DISCOVER_SNAP):
        ...
        - chip_sel: ADC chips be used (Default for Discover SNAP is chip 0. chip_sel=None
            will use all chips.)
        """
        self.host = host
    ...
    def alignFrameClock(self, chipsel=chip_sel, chip_lanes=None, retry=True):
        """
        ...
        if len(failed_chips) > 0 and retry:
            if self.s.adc._retry_cnt < self.s.adc._retry:
                ...
                self.alignLineClock(chipsel=failed_chips) # retry
                return self.alignFrameClock(chipsel=failed_chips) # retry
        return failed_chips
    ...
    def rampTest(self, chipsel=self.chip_sel, nchecks=300, retry=False):
        ...
        Inputs:
        - chipsel (list): which ADC chip to align.
        ...
        if len(failed_chips) > 0 and retry:
            if self.s.adc._retry_cnt < self.s.adc._retry:
                ...
                logging.info('retry=%d/%d redo Line/Frame on ADCs/lanes: %s' % \
                                (self.s.adc._retry_cnt, self.s.adc._retry, failed_chips))
                self.alignLineClock(failed_chips) # retry
                self.alignFrameClock(failed_chips) # retry
                return self.rampTest(chipsel=chipsel, nchecks=nchecks, retry=retry) # retry
        return failed_chips
    ...
    def align_adc(self, chipsel=self.chip_sel, chip_lanes=None, ker_size=5, retry=True, nchecks=300, force=False, verify=True):
        ...
        if self.s.adc_is_configured():
            return
        fails = self.alignLineClock(chipsel=chipsel, chip_lanes=chip_lanes, ker_size=ker_size)
        if len(fails) > 0:
            logging.warning("alignLineClock failed on: " + str(fails))
        fails = self.alignFrameClock(chipsel=chipsel, chip_lanes=chip_lanes, retry=retry)
        if len(fails) > 0:
            logging.warning("alignFrameClock failed on: " + str(fails))
        fails = self.rampTest(chipsel=chipsel, nchecks=nchecks, retry=False)
        if len(fails) > 0:
    ...
    def initialize(self, chipsel=self.chip_sel, chip_lanes=None, ker_size=5, retry=True, nchecks=300, force=False, verify=True):
        ...
        while self.s.adc_is_configured() == 0:
            self.s.adc.init()
            self.align_adc(chipsel=chipsel,
